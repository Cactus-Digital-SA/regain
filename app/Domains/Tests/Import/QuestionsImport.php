...
namespace App\Domains\Tests\Import;

use App\Domains\Language\Repositories\Eloquent\EloqLanguageRepository;
use App\Domains\Language\Services\LanguageService;
use App\Domains\Tests\Import\Sheets\PreAssessmentsImport;
use App\Domains\Tests\Import\Sheets\ReferenceImport;
use App\Domains\Tests\Import\Sheets\SkillsImport;
use App\Domains\Tests\Import\Sheets\SociodemographicImport;
use App\Domains\Tests\Jobs\CreateQuestionFromJob;
...
class QuestionsImport implements WithMultipleSheets
{
    ...
    public static function createInstruction(string $instruction, int $language_id): ?int{
        if($instruction != null && $instruction != '-') {
            $repository = new EloqInstructionRepository(new \App\Domains\Tests\Repositories\Eloquent\Models\Instruction());
    ...
    {
        $repository = new EloqLanguageRepository(new \App\Domains\Language\Repositories\Eloquent\Models\Language());
    ...
    public static function createCategory(string $category) : int
    {
        try {
            ...
            $category = $categoryService->firstOrCreate($category, null);
    ...
    public static function createTest(string $test,int $categoryId) : int
    {
        try {
            ...
            $testDTO = $testService->findOrCreate($test, $categoryId);
    ...
    public static function createSubscale(string $subscale, int $testId) : ?int
    {
        if($subscale != null && $subscale != '-') {
            try {
                ...
                $subscaleDTO = $subscaleService->findOrCreate($subscale, $testId);
    ...
    public static function createResponses(array $responses, int $language_id) : array
    {
        try {
            ...
            foreach ($responses as $response) {
                $responsesIds[] = $service->findOrCreate($response, $language_id)->getId();
            }
            return $responsesIds;
        } catch (Exception $exception) {
            Log::error($exception->getMessage());
